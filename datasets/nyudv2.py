...
class Dataset(Dataset):
    def __init__(self, path, areas, rate=1.0, flip_prob=None, crop_type=None, crop_size=0):
        ...
        data_path = '/work/kabelitz/data/NYU_Depth_V2/'
        ...
        # as it turns out, trying to pickle this is a bad idea :D
    ...
    def __getitem__(self, idx):
        rgb = self.rgb_images[idx][:, 1:].astype(np.float32)
        hha = np.transpose(cv2.imread("/work/kabelitz/data/NYU_Depth_V2/data/hha/img_" + str(idx+5001) + ".png", cv2.COLOR_BGR2RGB), [1, 0, 2])[:, 1:]
        rgb_hha = np.concatenate([rgb, hha], axis=2).astype(np.float32)
        label = self.label_images[idx][:, 1:].astype(np.float32)
